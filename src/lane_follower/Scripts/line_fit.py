...
def final_viz(undist, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset):
	...
	result = cv2.putText(result, label_str, (30,40), 0, 0.7, (0,0,0), 2, cv2.LINE_AA)

	label_str = 'Vehicle offset from lane center: %.3f m' % vehicle_offset
	result = cv2.putText(result, label_str, (30,70), 0, 0.7, (0,0,0), 2, cv2.LINE_AA)

	label_str = 'Right lane radius:' + format(right_curve, ".2f") + 'm Left:' + format(left_curve, ".2f")
	#result = cv2.putText(result, label_str, (30, 100), 0, 1, (0, 0, 0), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, thickness=1)
	result = cv2.putText(result, label_str, (30, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2, cv2.LINE_AA)
